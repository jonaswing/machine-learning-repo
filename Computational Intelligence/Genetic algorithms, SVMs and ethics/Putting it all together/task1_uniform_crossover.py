...
def breed_by_uniform_crossover(parent_1, parent_2):
    # Determine the length of the chromosome
    chromosome_length = len(parent_1)

    # Initialize children as arrays of zeros
    child_1 = np.zeros(chromosome_length)
    child_2 = np.zeros(chromosome_length)

    # Iterate through each gene in the parents
    for i in range(chromosome_length):
        # Randomly select whether the gene comes from parent 1 or parent 2
...
def genetic_algorithm(reference_solution,
    ...
    while generation < maximum_generation and best_score < fitness_target:
        ...
        for i in range(int(population_size / 2)):
            ...
            # function.
            child_1, child_2 = breed_by_uniform_crossover(parent_1, parent_2)
