...
def select_individual_by_random(population, scores):
    # Determine the initial size of the population using the length
    # of the scores array.
    population_size = len(scores)

    # Randomly select an individual from the population based on their fitness scores.
    selected_index = random.choices(range(population_size), weights=scores, k=1)[0]
...
def genetic_algorithm(reference_solution,
    ...
    while generation < maximum_generation and best_score < fitness_target:
        ...
        for i in range(int(population_size/2)):
           ...
           parent_1 = select_individual_by_random(population, scores)
           parent_2 = select_individual_by_random(population, scores)
